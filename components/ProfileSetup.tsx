...
export default function ProfileSetup({
  ...
  const [sessionTime, setSessionTime] = useState<PreferredTime | undefined>(
    initialSessionTime,
  );
  ...
  return (
    <div className="w-full max-w-sm space-y-6">
      <div className="text-center">
        <h2 className="text-2xl font-bold text-on-surface-light dark:text-on-surface">
          {t.title}
        </h2>
      ...
      <div>
        ...
        <div className="flex gap-2">
          {SESSION_TIMES.map((s) => (
            <OptionButton
              key={s.value}
              selected={sessionTime === s.value}
              onClick={() => setSessionTime(s.value)}
            >
              {s.label}
            </OptionButton>
          ))}
