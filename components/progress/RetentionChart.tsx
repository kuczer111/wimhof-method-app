  ...
  return (
    <Card>
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`rounded-md px-2 py-0.5 text-xs font-medium transition-colors ${mode === 'avg' ? 'bg-brand/10 text-brand-dark dark:bg-brand-dark/20 dark:text-brand-light' : 'text-on-surface-muted hover:text-on-surface-faint dark:hover:text-on-surface'}`}
          >
          ...
          <button
            ...
            className={`rounded-md px-2 py-0.5 text-xs font-medium transition-colors ${mode === 'rounds' ? 'bg-accent/10 text-accent-dark dark:bg-accent-dark/20 dark:text-accent-light' : 'text-on-surface-muted hover:text-on-surface-faint dark:hover:text-on-surface'}`}
          >
        ...
        {mode === 'avg' ? (
          <>
            <span className="flex items-center gap-1">
              <span className="inline-block h-0.5 w-3 bg-brand-light" />{' '}
              {strings.progress.chart.avgLegend}
            </span>
            <span className="flex items-center gap-1">
              <span
                className="inline-block h-0.5 w-3 bg-success-light"
                style={{ borderTop: '2px dashed #4ade80' }}
              />{' '}
              {strings.progress.chart.trendLegend}
            </span>
            <span className="flex items-center gap-1">
              <span
                className="inline-block h-0.5 w-3 bg-accent-light"
                style={{ borderTop: '2px dotted #fb923c' }}
              />{' '}
              {strings.progress.chart.weeklyAvgLegend}
            </span>
            <span className="flex items-center gap-1">
              <span className="inline-block h-2 w-2 rounded-full bg-warning-light" />{' '}
              {strings.progress.chart.prLegend}
            </span>
          </>
        ) : (
      ...
      <div className="sr-only">
        Retention chart showing {data.length} sessions. Overall average: {avg}{' '}
        seconds. Personal record: {prValue} seconds.
        {mode === 'rounds'
      ...
      <ResponsiveContainer width="100%" height={200}>
        <LineChart
          ...
          role="img"
          aria-label={`Retention chart: ${data.length} sessions, average ${avg}s, PR ${prValue}s`}
        >
          <XAxis
            dataKey="label"
