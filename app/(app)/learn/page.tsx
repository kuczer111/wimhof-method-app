    ...
    return activeChapter.content.split('\n\n').map((block, i) => {
      ...
      if (block.startsWith('**') && block.endsWith('**')) {
        return (
          <h3
            key={i}
            className="mt-6 mb-3 text-lg font-semibold text-on-surface-light dark:text-on-surface"
          >
            {block.slice(2, -2)}
          </h3>
        );
      ...
      return (
        <p
          key={i}
          className="mb-4 leading-relaxed text-on-surface-light-muted dark:text-on-surface-muted"
        >
          {parts.map((part, j) =>
            part.startsWith('**') && part.endsWith('**') ? (
              <strong
                key={j}
                className="font-semibold text-on-surface-light dark:text-on-surface"
              >
                {part.slice(2, -2)}
              </strong>
            ) : (
