...
async function ensureProgramProgressCache(): Promise<Map<string, ProgramProgress>> {
  if (programProgressCache) return programProgressCache;
  const db = await getDB();
  const all = await db.getAll("program_progress");
  programProgressCache = new Map(all.map((p) => [p.programId, p as ProgramProgress]));
  return programProgressCache;
}

export async function getProgramProgress(programId: string): Promise<ProgramProgress | undefined> {
...
export async function saveProgramProgress(progress: ProgramProgress): Promise<void> {
  const cache = await ensureProgramProgressCache();
